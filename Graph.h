...
public:
  ...
  if (airportIndex == -1) {
    addAirport(origin);
    connections.back().push_back(Edge(distance, cost, origin, destination));
    return;
  }

  connections[airportIndex].push_back(Edge(distance, cost, origin, destination));
}
    ...
    for (int i = 0; i < connections.size(); i++){
      ...
      for(int j = 0; j < currConn.size(); j++){
          ...
          if(connection.distance == 0 ){
              Edge newEdgeReverse = Edge(currEdge.distance, currEdge.cost, currEdge.destination, currAirport);
